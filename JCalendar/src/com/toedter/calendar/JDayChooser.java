...
public class JDayChooser extends JPanel implements ActionListener, KeyListener, FocusListener {
	...
	public JDayChooser(boolean weekOfYearVisible) {
		...
		for (int i = 0; i < 7; i++) {
			...
			weeks[i].addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {					
					...
					int oldWeek = week;
					week = new Integer(buttonText).intValue();
					firePropertyChange("week", oldWeek, week);
				}
			});
	 ...
	 */
	/*
	 * Code below commented out by Mark Brown on 24 Aug 2004. This code breaks the JDateChooser code by triggering the actionPerformed method on the next day button. This causes the date chosen to
	 * always be incremented by one day.
	 */
	 ...
	 * 
	 * @param min the minimum selectable date or null (then the minimum date is set to 01\01\0001)
	 * @param max the maximum selectable date or null (then the maximum date is set to 01\01\9999)
	 */
	...
	/**
	 * Sets the maximum number of characters per day in the day bar. Valid values are 0-4. If set to 0, dateFormatSymbols.getShortWeekdays() will be used, otherwise theses strings will be reduced to
